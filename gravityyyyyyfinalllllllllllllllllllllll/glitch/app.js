...
async function showApp() {
    ...
    await loadVaultData();
    loadNotifications();
    // Check URL hash for deep linking (e.g. #vault from Chrome extension)
    const hash = window.location.hash.replace('#', '');
    const validPages = ['dashboard', 'vault', 'autofill', 'scanner', 'consent', 'audit'];
    const targetPage = validPages.includes(hash) ? hash : 'dashboard';
    navigateTo(targetPage, document.querySelector(`[data-page="${targetPage}"]`));
}
